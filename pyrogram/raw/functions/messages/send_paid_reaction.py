...
class SendPaidReaction(TLObject["raw.base.Updates"]):
    ...
    Parameters:
        ...
        random_id (``int`` ``64-bit``):
            Unique client message ID required to prevent message resending. Note: this argument must be composed of a 64-bit integer where the lower 32 bits are random, and the higher 32 bits are equal to the current unixtime, i.e. `uint64_t random_id = (time() << 32)
