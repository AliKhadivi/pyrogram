...
class StarGift(TLObject):
    ...
    Parameters:
        ...
        peer_color_available (``bool``, *optional*):
            If set, collectible gifts of this type may be used to generate a message color palette and pattern ».

        auction (``bool``, *optional*):
            If set, this is a collectible gift that can only be bought through a collectible gift auction ».
        ...
        auction_slug (``str``, *optional*):
            Always set for gifts that can be bought on auctions », contains the auction deep link slug ».

        gifts_per_round (``int`` ``32-bit``, *optional*):
            Always set for gifts that can be bought on auctions », contains the number of gifts of this kind that are distributed on every round.

        auction_start_date (``int`` ``32-bit``, *optional*):
            Always set for gifts that can be bought on auctions », contains the UNIX timestamp indicating when will the auction start (or when the auction started, if it points to the past).
        ...
        background (:obj:`StarGiftBackground <pyrogram.raw.base.StarGiftBackground>`, *optional*):
            Default background palette for this gift type, used when rendering gift cards and previews before a specific collectible backdrop is chosen.
