...
class MessageEntityFormattedDate(TLObject):
    ...
    Parameters:
        ...
        date (``int`` ``32-bit``):
            The date as a UNIX timestamp: the allowed value ranges from 0 to the current date plus 1098 days (time()+1098*86400).

        relative (``bool``, *optional*):
            If set, render a relative date, see here » for more info. Cannot be combined with any of the other flags.

        short_time (``bool``, *optional*):
            Renders the time in short format: hours and minutes, according to the user's locale settings; cannot be combined with long_time.

        long_time (``bool``, *optional*):
            Renders the time in long format: hours, minutes, seconds and maybe timezone, according to the user's locale settings; cannot be combined with short_time.

        short_date (``bool``, *optional*):
            Renders the time in short format: month, date, (and year, if different from the current one), all according the user's locale settings; cannot be combined with long_date.

        long_date (``bool``, *optional*):
            Renders the time in long format: always month, date and year, all according the user's locale settings; cannot be combined with short_date.
