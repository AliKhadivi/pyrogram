...
class GroupCallParticipant(TLObject):
    ...
    Parameters:
        peer (:obj:`Peer <pyrogram.raw.base.Peer>`):
            Peer represented by this participant

        date (``int`` ``32-bit``):
            When did this participant join the group call

        source (``int`` ``32-bit``):
            Source ID of the participant's main audio stream

        muted (``bool``, *optional*):
            Whether the participant is muted

        left (``bool``, *optional*):
            Whether the participant left the media layer; in conferences, another participant must then prune them from the E2E blockchain
        ...
        versioned (``bool``, *optional*):
            If set, the application logic for this update is slightly different, see here » for more info on the full flow.
        ...
        volume (``int`` ``32-bit``, *optional*):
            Volume, between 1 and 20000; 10000 represents 100% volume. If not set, the volume is set to 100%.
        ...
        paid_stars_total (``int`` ``64-bit``, *optional*):
            Total Stars donated by this participant in a live story
