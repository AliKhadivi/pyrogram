...
class GroupCallMessage(TLObject):
    ...
    Parameters:
        ...
        from_id (:obj:`Peer <pyrogram.raw.base.Peer>`):
            Displayed message author

        date (``int`` ``32-bit``):
            Message date

        message (:obj:`TextWithEntities <pyrogram.raw.base.TextWithEntities>`):
            Message text or emoji reaction; empty for standalone paid live story donations. Must be at most group_call_message_length_limit » UTF-8 characters long. For paid chats, use stars_groupcall_message_limits » to source limits according to the passed value of paid_message_stars, instead.

        from_admin (``bool``, *optional*):
            Whether the message was sent by a group call admin

        paid_message_stars (``int`` ``64-bit``, *optional*):
            Number of Telegram Stars donated with the message or standalone donation
