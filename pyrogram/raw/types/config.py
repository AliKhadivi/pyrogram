...
class Config(TLObject):
    ...
    Parameters:
        ...
        edit_time_limit (``int`` ``32-bit``):
            Only messages with age (in seconds) smaller than the one specified in this field can be edited
        ...
        tmp_sessions (``int`` ``32-bit``, *optional*):
            Number of parallel sessions the client may open to the main connection of its home DC to increase throughput; if absent or ≤ 1, a single main session must be used
