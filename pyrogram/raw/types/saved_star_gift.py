...
class SavedStarGift(TLObject):
    ...
    Parameters:
        ...
        drop_original_details_stars (``int`` ``64-bit``, *optional*):
            If set, the starGiftAttributeOriginalDetails attribute of this gift may be removed by paying the specified amount of stars, see here » for the full flow.

        gift_num (``int`` ``32-bit``, *optional*):
            Collectible number assigned to the gift, if already known.

        can_craft_at (``int`` ``32-bit``, *optional*):
            If set, this gift can be used for crafting » only starting from the specified unixtime.
