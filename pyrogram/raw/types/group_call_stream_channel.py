...
class GroupCallStreamChannel(TLObject):
    ...
    Parameters:
        channel (``int`` ``32-bit``):
            Stream channel ID, used as a source of the synthetic unified video source group and as inputGroupCallStream.video_channel when fetching the corresponding video segment

        scale (``int`` ``32-bit``):
            Specifies the duration of the media segment to fetch. To obtain this value in milliseconds, bitshift 1000 to the right scale times: duration_ms := 1000 >> scale.
