...
class UpdateBotGuestChatQuery(TLObject):
    ...
    Parameters:
        query_id (``int`` ``64-bit``):
            Query identifier, to be passed to messages.setBotGuestChatResult when replying

        message (:obj:`Message <pyrogram.raw.base.Message>`):
            The message that triggered the query

        qts (``int`` ``32-bit``):
            Persistent timestamp (qts) of the update, used by bots to receive updates of this type

        reference_messages (List of :obj:`Message <pyrogram.raw.base.Message>`, *optional*):
            Additional context messages referenced by the triggering message (for example replied-to messages)
