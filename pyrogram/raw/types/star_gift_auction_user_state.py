...
class StarGiftAuctionUserState(TLObject):
    ...
    Parameters:
        acquired_count (``int`` ``32-bit``):
            Contains the number of gifts that were purchased so far in the auction by the current user.

        returned (``bool``, *optional*):
            If set, the placed bid was returned to the user, because it was outbid so much that it fell out of the top starGiftAuctionState.gifts_left positions, meaning that even if no new bids are placed, the user will never receive any gifts, so the bid was completely removed from the auction, and in order to participate again the user must manually make a new bid.
        ...
        min_bid_amount (``int`` ``64-bit``, *optional*):
            Contains the minumum allowed bid amount in Telegram Stars, if set overrides starGiftAuctionState.min_bid_amount for the current user.
