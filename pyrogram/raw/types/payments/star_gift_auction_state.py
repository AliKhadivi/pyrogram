...
class StarGiftAuctionState(TLObject):
    ...
    Parameters:
        ...
        user_state (:obj:`StarGiftAuctionUserState <pyrogram.raw.base.StarGiftAuctionUserState>`):
            Auction state related to the current user (i.e. info about placed bids, won gifts, and so on).

        timeout (``int`` ``32-bit``):
            Re-invoke the payments.getStarGiftAuctionState method after timeout seconds to keep receiving auction updates, see here » for more info on the full flow.
