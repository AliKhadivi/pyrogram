...
class InputPasskeyResponseLogin(TLObject):
    ...
    Parameters:
        client_data (:obj:`DataJSON <pyrogram.raw.base.DataJSON>`):
            Base64url-decoded clientDataJSON field of an AuthenticatorAssertionResponse, wrapped in dataJSON
        ...
        user_handle (``str``):
            Base64url-decoded userHandle field of an AuthenticatorAssertionResponse. The user handle specified by the server is in <dc_id>:<user_id> format.
