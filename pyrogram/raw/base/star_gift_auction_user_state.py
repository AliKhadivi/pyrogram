...
if TYPE_CHECKING:
    StarGiftAuctionUserState = Any
else:
    StarGiftAuctionUserState = Union[raw.types.StarGiftAuctionUserState]

_doc = """Contains information about the current user's state in an auction ».
...
try:
    _t = type(StarGiftAuctionUserState)
    ...
    if _module != "typing" and not (_module == "types" and _name == "UnionType"):
        StarGiftAuctionUserState.__doc__ = _doc
