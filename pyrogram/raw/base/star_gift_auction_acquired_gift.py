...
if TYPE_CHECKING:
    StarGiftAuctionAcquiredGift = Any
else:
    StarGiftAuctionAcquiredGift = Union[raw.types.StarGiftAuctionAcquiredGift]

_doc = """Describes a gift that the current user won in an auction.
...
try:
    _t = type(StarGiftAuctionAcquiredGift)
    ...
    if _module != "typing" and not (_module == "types" and _name == "UnionType"):
        StarGiftAuctionAcquiredGift.__doc__ = _doc
